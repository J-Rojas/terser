...
export function mangle_helpers(options, reserved, cache) {
    ...
    var mangleRegex = options.mangle_pattern && new RegExp(options.mangle_pattern);
    ...
    function approved_mangle(name) {
        // mangle pattern test
        if (mangleRegex && !mangleRegex.test(name)) return false;
        return true;
    }
    ...
    function mangle(name, ignoreShould) {
        if (!ignoreShould && !should_mangle(name)) {
            return name;
        }
        ...
        if (!mangled) {
            ...
            if (!mangled) {
                ...
                } while (!can_mangle(mangled) || !approved_mangle(mangled));
            }
